...
// use cesu8::{Cesu8DecodingError, from_cesu8, from_java_cesu8};
use clap::Clap;
...
fn real_main() -> i32 {
	...
	loop {
		...
		let output_data: Cow<[u8]> = if !opts.decode { // encode into CESU8/MUTF8
			...
			let encoded = if opts.java {
				cesu8::to_cesu8(as_str)
			} else {
				cesu8::to_java_cesu8(as_str)
			};
