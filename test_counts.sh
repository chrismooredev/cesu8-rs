...
cargo build --bin cesu8 --features "build-binary" --quiet 2>/dev/null

echo "following hashes should be the same..."
./target/debug/cesu8 test_files/random.txt 2>/dev/null | md5sum test_files/random.cesu8.txt /dev/stdin
